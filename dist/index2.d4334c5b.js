...
},{"./lib/axios":"node_modules/axios/lib/axios.js"}],"index2.js":[function(require,module,exports) {
  ...
  _createClass(Table, [{
    ...
    value: function setUpEvent() {
      ...
      this.$mainForm.addEventListener('submit', function (e) {
        e.preventDefault();
        var name = _this.$mainForm.elements.name.value;
        var age = _this.$mainForm.elements.age.value;
        var address = _this.$mainForm.elements.address.value;
        var phone = _this.$mainForm.elements.phone.value;
        var user = {};
        user.name = name;
        user.age = age;
        ...
        if (_this.editingId === null) {
          axios.post(URL, user).then(function (res) {
            //promise resolve;
            _this.users.push(res.data);

            _this.addTableRow(res.data);

            _this.$showMsg.className = 'text-success';
            _this.$showMsg.classList.add = 'mt-5';
            _this.$showMsg.innerHTML = 'User Created Successfully'; // this.reRenderTable();
          }).catch(function (err) {
            _this.$showMsg.className = 'text-danger';
            _this.$showMsg.innerHTML = 'unable to create user';
          });

          _this.resetForm();
        } else {
          axios.put(URL + '/' + _this.editingId, user).then(function (res) {
            _this.editingId = null;

            var tempUser = _this.users.find(function (user) {
            ...
            _this.$showMsg.className = 'text-success';
            _this.$showMsg.classList.add = 'mt-5';
            _this.$showMsg.innerHTML = 'User is edited Successfully';
          }).catch(function (err) {
            _this.$showMsg.className = 'text-danger';
            _this.$showMsg.innerHTML = 'unable to create user';
          });
    ...
    key: "updateData",
    value: function updateData(element) {
      //console.log(element);
      var currentUser = this.users.find(function (user) {
        return user.id == element.dataset.id;
      });
    ...
    key: "deleteData",
    value: function deleteData(element) {
      var _this2 = this;

      axios.delete(URL + '/' + element.dataset.id).then(function () {
        _this2.users = _this2.users.filter(function (usr) {
          return usr.id !== +element.dataset.id;
        });

        _this2.reRenderTable();

        _this2.$showMsg.className = 'text-danger';
        _this2.$showMsg.innerHTML = 'user deleted';
      });
    }
  }, {
    key: "searchData",
    ...
    key: "addTableRow",
    value: function addTableRow(user) {
    ...
    value: function reRenderTable() {
      ...
      if (this.$search.value !== '') {
        filteredUsers = filteredUsers.filter(function (user) {
          return user.name.includes(_this4.$search.value);
        });
      }
    ...
    value: function getFormData() {
      ...
      axios.get(URL).then(function (res) {
        ...
        _this5.reRenderTable(); // first time run of runRenderTable; 
...
},{}]},{},["../../AppData/Roaming/npm/node_modules/parcel-bundler/src/builtins/hmr-runtime.js","index2.js"], null)
